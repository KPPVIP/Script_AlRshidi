local Config = module('Script_AlRshidi', 'config')
local Locales = module('Script_AlRshidi', 'locales/languages')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		TriggerServerEvent('Script_AlRshidi:getServerInfo')
...
-- Overall Info
RegisterNetEvent('Script_AlRshidi:setInfo')
AddEventHandler('Script_AlRshidi:setInfo', function(info)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('Script_AlRshidi:setCarSignalLights', vehicleSignalIndicator)
		end
...
AddEventHandler('Script_AlRshidi:setCarSignalLights', function(status)
	...
	TriggerServerEvent('Script_AlRshidi:syncCarLights', status)
...
RegisterNetEvent('Script_AlRshidi:syncCarLights')
AddEventHandler('Script_AlRshidi:syncCarLights', function(driver, status)
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'id' })
		...
		if (Config.ui.showid == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'id' })
		end
